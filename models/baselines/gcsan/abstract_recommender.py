...
from utils import ModelType, InputType, FeatureSource, FeatureType, set_color

class FMEmbedding(Model):
...
class FMFirstOrderLinear(Model):
    ...
    def __init__(self, config, dataset, output_dim=1):
        ...
        if len(self.float_field_dims) > 0:
            self.float_embedding_table = Embedding(np.sum(self.float_field_dims, dtype=np.int32), output_dim)
        if len(self.token_seq_field_dims) > 0:
            self.token_seq_embedding_table = tf.TensorArray()
            for token_seq_field_dim in self.token_seq_field_dims:
                self.token_seq_embedding_table.append(Embedding(token_seq_field_dim, output_dim))

        self.bias = tf.zeros((output_dim,), requires_grad=True)
    ...
    def embed_float_fields(self, float_fields, embed=True):
        ...
        index = tf.range(0, num_float_field).unsqueeze(0).expand_as(float_fields).long().to(self.device)

        # [batch_size, num_float_field, output_dim]
        float_embedding = self.float_embedding_table(index)
        float_embedding = tf.multiply(float_embedding, float_fields.unsqueeze(2))

        # [batch_size, 1, output_dim]
        float_embedding = tf.reduce_sum(float_embedding, axis=1, keepaxis=True)
    ...
    def embed_token_fields(self, token_fields):
        ...
        # [batch_size, 1, output_dim]
        token_embedding = tf.reduce_sum(token_embedding, axis=1, keepaxis=True)
    ...
    def embed_token_seq_fields(self, token_seq_fields):
        ...
        for i, token_seq_field in enumerate(token_seq_fields):
            ...
            mask = mask.float()
            value_cnt = tf.reduce_sum(mask, axis=1, keepaxis=True)  # [batch_size, 1]
            ...
            result = tf.reduce_sum(masked_token_seq_embedding, axis=1, keepaxis=True)  # [batch_size, 1, output_dim]

            fields_result.append(result)
        if len(fields_result) == 0:
            return None
        else:
            return tf.reduce_sum(tf.concat(fields_result, axis=1), axis=1, keepaxis=True)  # [batch_size, 1, output_dim]
    ...
    def forward(self, interaction):
        ...
        if len(float_fields) > 0:
            float_fields = tf.concat(float_fields, axis=1)  # [batch_size, num_float_field]
        else:
            float_fields = None
        ...
        if len(token_fields) > 0:
            token_fields = tf.concat(token_fields, axis=1)  # [batch_size, num_token_field]
        else:
            token_fields = None
        # [batch_size, 1, output_dim] or None
        ...
        return tf.reduce_sum(tf.concat(total_fields_embedding, axis=1), axis=1) + self.bias  # [batch_size, output_dim]

class AbstractRecommender(Model):
...
class SequentialRecommender(AbstractRecommender):
    ...
    def gather_indexes(self, output, gather_index):
        ...
        output_tensor = output.gather(axis=1, index=gather_index)
    ...
    def get_attention_mask(self, item_seq, bidirectional=False):
        ...
        if not bidirectional:
            extended_attention_mask = tf.linalg.band_part(extended_attention_mask.expand((-1, -1, item_seq.size(-1), -1)), -1, 0)
        extended_attention_mask = tf.where(extended_attention_mask, 0., -10000.)
...
class ContextRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if len(self.float_field_dims) > 0:
            self.float_embedding_table = Embedding(
                np.sum(self.float_field_dims, dtype=np.int32), self.embedding_size
            )
        if len(self.token_seq_field_dims) > 0:
            self.token_seq_embedding_table = tf.TensorArray()
            for token_seq_field_dim in self.token_seq_field_dims:
                self.token_seq_embedding_table.append(Embedding(token_seq_field_dim, self.embedding_size))
    ...
    def embed_token_seq_fields(self, token_seq_fields, mode='mean'):
        ...
        for i, token_seq_field in enumerate(token_seq_fields):
            ...
            if mode == 'max':
                ...
                result = tf.math.reduce_max(masked_token_seq_embedding, axis=1, keepaxis=True)  # [batch_size, 1, embed_dim]
            elif mode == 'sum':
                masked_token_seq_embedding = token_seq_embedding * mask.float()
                result = tf.reduce_sum(masked_token_seq_embedding, axis=1, keepaxis=True)  # [batch_size, 1, embed_dim]
            else:
                masked_token_seq_embedding = token_seq_embedding * mask.float()
                result = tf.reduce_sum(masked_token_seq_embedding, axis=1)  # [batch_size, embed_dim]
                eps = tf.constant([1e-8]).to(self.device)
                result = tf.math.divide(result, value_cnt + eps)  # [batch_size, embed_dim]
                result = result.unsqueeze(1)  # [batch_size, 1, embed_dim]
            fields_result.append(result)
        if len(fields_result) == 0:
            return None
        else:
            return tf.concat(fields_result, axis=1)  # [batch_size, num_token_seq_field, embed_dim]
    ...
    def double_tower_embed_input_fields(self, interaction):
        ...
        if dense_embedding is not None:
            first_dense_embedding, second_dense_embedding = \
                tf.split(dense_embedding, [self.user_float_field_num, self.item_float_field_num], axis=1)
        else:
        ...
        if sparse_embedding is not None:
            ...
            first_token_seq_embedding, second_token_seq_embedding, first_token_embedding, second_token_embedding = \
                tf.split(sparse_embedding, sizes, axis=1)
            first_sparse_embedding = tf.concat([first_token_seq_embedding, first_token_embedding], axis=1)
            second_sparse_embedding = tf.concat([second_token_seq_embedding, second_token_embedding], axis=1)
        else:
    ...
    def concat_embed_input_fields(self, interaction):
        ...
        return tf.concat(all_embeddings, axis=1)  # [batch_size, num_field, embed_dim]
    ...
    def embed_input_fields(self, interaction):
        ...
        if token_fields_embedding is None:
            ...
            if token_seq_fields_embedding is None:
                ...
                sparse_embedding = tf.concat([token_seq_fields_embedding, token_fields_embedding], axis=1)
