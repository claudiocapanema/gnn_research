...
class GNN(Model):
    ...
    def __init__(self, embedding_size, step=1):
        ...
        self.w_ih = self.add_weight((self.gate_size, self.input_size))
        self.w_hh = self.add_weight((self.gate_size, self.embedding_size))
        self.b_ih = self.add_weight((self.gate_size))
        self.b_hh = self.add_weight((self.gate_size))

        self.linear_edge_in = Dense(self.embedding_size, bias=True)
        self.linear_edge_out = Dense(self.embedding_size, bias=True)
    ...
    def GNNCell(self, A, hidden):
        ...
        input_in = tf.matmul(A[:, :, :A.size(1)], self.linear_edge_in(hidden))
        input_out = tf.matmul(A[:, :, A.size(1):2 * A.size(1)], self.linear_edge_out(hidden))
...
class GCSAN(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.item_embedding = Embedding(self.n_items, self.hidden_size, padding_idx=0)
        ...
        self.loss_fct = CategoricalCrossentropy()
        # else:
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (Dense, Embedding)):
        ...
        if isinstance(module, Dense) and module.bias is not None:
            module.bias.data.zero_()
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        seq_hidden = tf.gather(hidden, axis=1, index=alias_inputs)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            pos_score = tf.reduce_sum(seq_output * pos_items_emb, axis=-1)  # [B]
            neg_score = tf.reduce_sum(seq_output * neg_items_emb, axis=-1)  # [B]
            loss = self.loss_fct(pos_score, neg_score)
        else:  # self.loss_type = 'CE'
            test_item_emb = self.item_embedding.weight
            logits = tf.matmul(seq_output, test_item_emb.transpose(0, 1))
    ...
    def predict(self, interaction):
        ...
        scores = tf.math.multiply(seq_output, test_item_emb).sum(axis=1)  # [B]
        return scores
    ...
    def full_sort_predict(self, interaction):
        ...
        scores = tf.matmul(seq_output, test_items_emb.transpose(0, 1))  # [B, n_items]
        return scores